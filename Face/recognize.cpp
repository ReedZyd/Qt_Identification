...
SeetaFace::SeetaFace(){
    this->detector = new Detector("/home/wj/Qt/Qt_face/Face/model/seeta_fd_frontal_v1.0.bin");
        this->point_detector = new seeta::FaceAlignment("/home/wj/Qt/Qt_face/Face/model/seeta_fa_v1.1.bin");
        this->face_recognizer = new seeta::FaceIdentification("/home/wj/Qt/Qt_face/Face/model/seeta_fr_v1.0.bin");}
...
bool SeetaFace::GetFeature(cv::Mat color, float* feat){
    ...
    face_num = static_cast<int32_t>(faces.size());
	if (face_num == 0)
	{
		return false;
	}
    ...
    for (int32_t i = 0; i < face_num; i++)
  {
    ...
    int max_num=0;
    for(int32_t n =0; n<face_num;n++)
     {
       if(bbox[n].width>bbox[max_num].width)
         {
           max_num=n;
    ...
    this->point_detector->PointDetectLandmarks(src_img_data, faces[max_num], points);
